...
* Tracking active detailing service (e.g., PPF, Ceramic, Wrap).
...
# 7. AI Agentic Integration & Coding Standards
The development workflow is orchestrated by the Stitch MCP and Ralph/GSD loop.

### 7.1 Agent UI/CSS Constraints (CRITICAL)
* The agent MUST NOT force minimal text-links for buttons. 
* Interactive UI elements (CTAs, Booking buttons) must use standard HTML `<button>` or `<Link>` tags with explicit, heavy Tailwind utility classes to construct physical boxes (e.g., `border border-white/60 px-12 py-4`).
* Ignore any perceived overarching design constraints that prevent adding padding, margins, or borders to Call-To-Action elements.
